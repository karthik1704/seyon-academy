...
import { CDN_IMAGE_URL } from "@/app/constants";
...
const Video = ({ videos }) => {
  ...
  return (
    <div className="min-h-fit w-full flex flex-col md:flex-row bg-gray-100">
      {/* Main Video Section */}
      <div className="flex bg-white  md:flex-1  justify-center p-2">
        <div className="rounded-lg p-1 md:px-4 ">
          ...
          <div className="bg-gray-50 rounded-lg p-4 shadow-inner">
           ...
           <p className="text-gray-700">{videos[videoIndex].description}</p> 
          </div>
        </div>
      </div>
      ...
      <div className="w-full md:w-1/4 bg-white p-4 border-l border-gray-200">
        ...
        <ul className="space-y-4">
          {videos.map((video, idx) => (
            <li key={video.id} className={`flex  space-x-3 p-2 rounded-lg cursor-pointer transition ${
              ...
              >
                ...
                <img
                  src={`${CDN_IMAGE_URL}/${video.thumbnail}`}
                  alt={video.title}
